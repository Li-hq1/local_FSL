...
def train_one_epoch(local_model: torch.nn.Module, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # localization
        with torch.no_grad():
            x_logits, tscams = local_model(samples)
            B, C, H, W = tscams.shape
            x_probs = F.softmax(x_logits, dim=-1)
            pred_cls_id = x_probs.argmax(dim=1)
            # find cams feature map of the correct class
            keep_pred = torch.gather(tscams, dim=1, index=pred_cls_id[:, None, None, None].expand(B,1,H,W)).squeeze(1)
            keep_pred = rearrange(keep_pred, "B H W -> B (H W)")

            keep_min_v, keep_max_v = keep_pred.min(dim=1, keepdim=True)[0], keep_pred.max(dim=1, keepdim=True)[0]
            keep_pred = (keep_pred - keep_min_v) / (keep_max_v - keep_min_v)

            threshold = 0.3
            keep_pred[keep_pred<threshold] = 0.2
            keep_pred[keep_pred>=threshold] = 1 # [B P]


        with torch.cuda.amp.autocast():
            outputs = model(samples, keep_pred)
