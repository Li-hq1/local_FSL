...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def random_masking(self, x, mask_ratio):
        B, L, D = x.size()
        len_keep = int(L * (1 - mask_ratio)) if self.training else L
    ...
    def forward_features(self, x):
        ...
        if self.pooling == 'mean':
            ...
            zero_tokens = torch.zeros([B, ids_restore.shape[1] - x.shape[1], D], device=x.device)
            x = torch.cat([x, zero_tokens], dim=1)  # no cls token
