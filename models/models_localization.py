...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, drop_path_rate=0., num_classes=1000):
        super().__init__()
    ...
    def forward(self, x):
        ...
        n, p, c = x_patch.shape
        x_patch = torch.reshape(x_patch, [n, int(p**0.5), int(p**0.5), c]) # [n h w c]
        x_patch = x_patch.permute([0, 3, 1, 2]) # [n c h w]
        x_patch = x_patch.contiguous()
        x_patch = self.head(x_patch) # [n c2 h w]
        x_logits = self.avgpool(x_patch).squeeze(3).squeeze(2) # [n c2]

        if self.training:
            return x_logits
        else:
            attn_weights = torch.stack(attn_weights)        # [12 n head p+1 p+1] p=hw
            attn_weights = torch.mean(attn_weights, dim=2)  # [12 n p+1 p+1]

            feature_map = x_patch.detach().clone()    # [n c2 h w]
            ...
            # sum [n p+1 p+1] -> cls [n p] -> reshp&unsqz [n 1 h w]
            cams = attn_weights.sum(0)[:, 0, 1:].reshape([n, h, w]).unsqueeze(1) 
            cams = cams * feature_map # [n 1 h w] * [n c2 h w]
